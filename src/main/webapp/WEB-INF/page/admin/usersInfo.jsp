...
<div class="container" style="width: 1200px">
    ...
    <h2 class="page-header text-left">会员列表</h2>
            ...
            <table class="table table-striped text-center">
                ...
                <c:forEach items="${page.records}" var="good">
                    <tr>
                        ...
                        <td><a class="btn btn-default" href="<%=request.getContextPath()%>/user/deleteUser?id=${good.id}">删除</a></td>
                    </tr>
                </c:forEach>
                </tbody>
            </table>
    ...
    <ul class="pagination">
        <c:if test="${page.pages > 1  && page.current > 1}">
            <li><a href="<%=request.getContextPath()%>/user/selectAllUser/${page.current - 1}">&laquo;</a></li>
        </c:if>

        <c:forEach var="i" begin="1" end="${page.pages}">
            <c:if test="${page.current == i}">
                <li class="active"><a href="<%=request.getContextPath()%>/user/selectAllUser/${i}">${i}</a></li>
            </c:if>
            <c:if test="${page.current != i}">
                <li class=""><a href="<%=request.getContextPath()%>/user/selectAllUser/${i}">${i}</a></li>
            </c:if>
        </c:forEach>
        <c:if test="${page.pages > 1 && page.current < page.pages}">
            <li><a href="<%=request.getContextPath()%>/user/selectAllUser/${page.current + 1}">&raquo;</a></li>
        </c:if>
